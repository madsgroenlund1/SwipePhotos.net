...
- Eye color must exactly match the customer's reference photos — check this carefully, it is a common mistake. Never default to a generic or the template model's eye color.
- The face must be a 1:1 match to the customer's own uploaded reference photos, not merely "similar" — recheck the customer's face-angle photos before finalizing and correct any drift in face shape, eye color, or proportions. Only the pose, body size and framing should follow image #1's model; the face identity itself must come entirely from the customer's own photos.
- If the customer has a clearly visible personal watch in his reference photos, reproduce that watch instead of any watch in image #1. Otherwise keep image #1's watch/accessories unchanged. Any watch, jewelry or other reflective accessory must look like real metal/glass with believable reflections and highlights — not flat, plastic, or artificially glossy.
