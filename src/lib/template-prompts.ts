...
export const TEMPLATE_PROMPTS: Record<string, StylePrompts> = {
  beach: build(
    ...
    'SPECIFIC AND REPEATED FAILURE — sunglasses being dropped: image #1 shows the model WEARING sunglasses over his eyes. This is a hard requirement, not optional: the customer in the result must also be wearing sunglasses, in the exact same position, style and shape as image #1 — on his face, over his eyes. Do not render him bare-eyed, do not remove the sunglasses, do not move them to his head or hang them off his shirt instead of wearing them. Before finishing, specifically check the eyes area of the result: if his eyes are visible and uncovered, that is an automatic failure and must be corrected. Even with sunglasses on, the result must still feel like the customer: match face shape, cheekbones, jawline, mouth shape, skin tone, hairline, hair color, and visible facial structure. Identity must be locked through these visible features exactly as precisely as if the eyes were shown.'
  ),
  restaurant: build(
  ...
  rooftop: build(
    ...
    `Important corrections for this specific template — read carefully, these are repeated failures and all must be fixed:

1. LEG / LOWER BODY: image #1 shows the customer's leg and trouser fully extending down to the bottom edge of the frame, resting naturally in the seat. This is a repeated and specific failure: the leg gets cut off or goes missing entirely near where the hand/finger rests in the frame, instead of continuing down naturally. Do NOT crop, shorten, or truncate the leg. Do NOT let the leg end early, fade out, or disappear above the bottom of the frame — including in the area right next to or below the hand/fingers. Reproduce the leg, trouser fabric, and its full length exactly as far down as image #1 shows it, all the way to the frame edge, anatomically continuous and with correct proportions matching the rest of the body. Before finishing, specifically trace the leg from hip to the bottom of the frame and confirm it is unbroken.
...
3. EYE COLOR (this specific template fails on it often): re-verify the customer's exact eye color from his reference photos one more time before finishing — this scene's blue-hour lighting tends to push the model toward rendering lighter/bluer eyes than the customer actually has. Correct for that: keep his eyes their true, real color regardless of the cool ambient lighting tone elsewhere in the scene.
