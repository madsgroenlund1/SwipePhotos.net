...
Within that locked head frame, reconstruct the customer's actual face with passport-photo precision using all uploaded customer references — not "similar," an exact match: eye color (use the customer's real, exact shade — brown/blue/green/hazel — never a generic or different color; this is the most common failure, check it carefully), face shape, eyebrows, nose, lips, cheekbones, chin, jawline, ears, skin tone, skin texture, hair color and hairline. Use the customer's full-body photo to match build and proportions, and the three face-angle photos to lock identity from multiple angles. Keep the customer looking in the same direction as the template. Do not idealise, slim or smooth the face — reproduce the exact real person, imperfections included. If a family member would hesitate to recognize him in the result, it has failed.
